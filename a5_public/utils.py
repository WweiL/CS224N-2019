...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = []
    longest_legth = max([len(s) for s in sents])
    pad_token = [char_pad_token] * max_word_length
    sents_word_padded = sents
    for s in sents_word_padded:
        sents_padded.append([w[:max_word_length] \
            if len(w) > max_word_length\
            else w + [char_pad_token] * (max_word_length - len(w))\
            for w in s] + \
            [pad_token for _ in range(longest_legth - len(s))])
    ### END YOUR CODE
...
def pad_sents(sents, pad_token):
    ...
    longest_legth = max([len(s) for s in sents])
    for sent in sents:
        if len(sent) < longest_legth:
            sent += [pad_token] * (longest_legth - len(sent))
