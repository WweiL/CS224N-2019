...
def question_1g_sanity_check():
    ...
    print("Running test on a list of sentences")
    sentences = [['Human:', 'What', 'do', 'we', 'want?'], ['Computer:', 'Natural', 'language', 'processing!'], ['Human:', 'When', 'do', 'we', 'want', 'it?'], ['Computer:', 'When', 'do', 'we', 'want', 'what?']]
    output_tensor = vocab.to_input_tensor_char(sentences, torch.device('cpu'))
    assert output_tensor.shape == torch.Size([6, len(sentences), 21]), "Tensor shape is incorrect: it should be:\n {} but is:\n{}".format([6, len(sentences), 21], output_tensor.shape)
...
def question_1h_sanity_check():
    ...
    input = torch.tensor(np.stack([i * ones for i in range(batch_size)])).float()
    model = Highway(embed_size)
    weight = torch.eye(embed_size)
    model.projection.weight = torch.nn.Parameter(weight)
    model.gate.weight = torch.nn.Parameter(weight)
    with torch.no_grad():
        output = model.forward(input)
    assert output.shape == torch.Size([batch_size, embed_size]), "Tensor shape is incorrect: it should be:\n {} but is:\n{}".format([batch_size, embed_size], output.shape)
...
def question_1i_sanity_check():
    ...
    m_word = 8
    ones = np.ones(m_word)
    input = torch.tensor(np.stack([[i * ones + j * 10 for i in range(embed_char_size)] for j in range(batch_size)])).float()
    model = CNN(num_channels, embed_char_size)
    torch.nn.init.constant_(model.conv.weight, 1)
    torch.nn.init.constant_(model.conv.bias, 1)
    with torch.no_grad():
        output = model.forward(input)
    assert output.shape == torch.Size([batch_size, num_channels]), "Tensor shape is incorrect: it should be:\n {} but is:\n{}".format([batch_size, num_channels], output.shape)
    # print(output)
